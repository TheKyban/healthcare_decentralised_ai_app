...
function formatTextStyling(text) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (segment.startsWith('**') && segment.endsWith('**')) {
      ...
      parts.push(<strong key={`bold-${i}-${boldText.substring(0, 10)}`}>{boldText}</strong>);
    } else {
      // Regular text
      parts.push(<span key={`text-${i}-${segment.substring(0, 10)}`}>{segment}</span>);
    }
  }
...
function formatAIResponse(text) {
  ...
  let formattedText = text.split('\n').map((line, i) => {
    // H3 headings
    if (line.startsWith('### ')) {
      return <h3 key={`h3-${i}-${line.substring(4, 20)}`} className="text-lg font-semibold mt-4 mb-2">{line.substring(4)}</h3>;
    }
    // H2 headings
    if (line.startsWith('## ')) {
      return <h2 key={`h2-${i}-${line.substring(3, 20)}`} className="text-xl font-bold mt-4 mb-2">{line.substring(3)}</h2>;
    }
    // Empty lines
    if (line.trim() === '') {
  ...
  formattedText = paragraphs.map((paragraph, i) => {
    ...
    if (paragraph.includes('- ') || paragraph.includes('* ')) {
      ...
      return (
        <ul key={`ul-${i}-${paragraph.substring(0, 20)}`} className="list-disc pl-5 space-y-1 my-2">
          {bulletItems.map((item, j) => (
            <li key={`li-${i}-${j}-${item.substring(0, 15)}`}>{formatTextStyling(item.replace(/^[\-\*] /, ''))}</li>
          ))}
        </ul>
      );
    }
    ...
    return <p key={`p-${i}-${paragraph.substring(0, 20)}`} className="my-2">{formatTextStyling(paragraph)}</p>;
  });
    ...
    try {
      ...
      while (true) {
        ...
        if (accumulatedText.includes('___METADATA___')) {
          const [textPart, metadataPart] = accumulatedText.split('___METADATA___');
      ...
      setStreamingMessage('');
      setConversationHistory(prev => [...prev, { role: 'assistant', content: finalText }].slice(-10));
